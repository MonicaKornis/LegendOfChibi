...
class Game {
  ...
  renderTimer(){
    ...
    if (this.gameOver){
      ...
      scoreSheet = `Your time is ${Math.floor(this.frames / 60)}:${Math.floor(this.frames % 60)}!`
    } else if (this.gamePaused){
      scoreSheet = " Game paused"
    } else {
      scoreSheet = `Time: ${Math.floor(this.frames / 60)}:${Math.floor(this.frames % 60)}
